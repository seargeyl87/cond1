...
function CatalogFilter({
  ...
  const [producerList, setProducerList] = useState([
    ...
    { cond: "Daikin", name: "daikin", id: 5 },
    { cond: "Mitsubishi Heavy", name: "mitsubishi_heavy", id: 6 },
    { cond: "Mitsubishi Electric", name: "mitsubishi_electric", id: 7 },
  ]);
            ...
            {invertorFilter.map((item) => (
              <div className={styles.filter_checkbox} key={item.id}>
                <input
                  ...
                  onChange={() => {
                    chooseInvertor == item.id
                      ? handleChangeInvertor(-1)
                      : handleChangeInvertor(item.id, item.choose);
                  }}
                  name={item.choose}
                />
                <p>{item.choose}</p>
